...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initMap() {
        ...
        paths.forEach(path => {
            ...
            path.addEventListener('click', (e) => {
                selectRegionMap(path.id, e.ctrlKey);
                e.stopPropagation();
            });
        });

        svgContainer.addEventListener('click', () => deselectRegions(true));
    }
    ...
    // Selecting region for map
    function selectRegionMap(regionId, isCtrlPressed) {
        if (!isCtrlPressed) {
        ...
        updateHighlightsAndFilters(true);
    }

    // Selecting region for table
    function selectRegionTable(regionId, isCtrlPressed) {
        if (!isCtrlPressed) {
            selectedRegions.clear();
        }
        ...
        updateHighlightsAndFilters(false);
    }

    function deselectRegions(applyFilter = true) {
        selectedRegions.clear();
        updateHighlightsAndFilters(applyFilter);
    }

    function updateHighlightsAndFilters(applyFilter = true) {
        highlightRegions();
        if (applyFilter) {
            filterTable();
        } else {
    ...
    function highlightRegions() {
        ...
        paths.forEach(path => {
            path.style.opacity = selectedRegions.has(path.id) ? '1' : '0.2';
        });
    }
    ...
    function updateTableRowOpacity() {
        ...
        rows.forEach(row => {
            row.style.opacity = selectedRegions.has(row.dataset.region) ? '1' : '0.2';
        });
    }
    ...
    function filterTable() {
        ...
        rows.forEach(row => {
            ...
            const matchesRegion = selectedRegions.size === 0 || selectedRegions.has(row.dataset.region);

            if (matchesInd && matchesNativity && matchesRegion) {
                row.style.display = '';
                row.style.backgroundColor = visibleRowIndex % 2 === 0 ? 'rgb(235, 235, 235)' : 'rgb(255, 255, 255)';
                visibleRowIndex++;
            } else {
    ...
    function populateTable(data) {
        ...
        data.forEach((item, index) => {
            ...
            row.addEventListener('click', (e) => {
                selectRegionTable(item.ID, e.ctrlKey);
            });
