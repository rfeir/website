...
// Update SVG path colors by reading precomputed hex color from data
function updatePathColors(data) {
    data.forEach(item => {
        const path = document.getElementById(item.ID);
        if (path) {
            path.style.fill = item.colorHex;
        }
    });
}
